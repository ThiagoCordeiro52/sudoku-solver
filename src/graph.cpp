...
void Graph::print_graph() const
{
  ...
  {
    std::cout << "Node " << nodes[i].value << " adjacents: ";
    for (int j = 0; j < RANK * RANK; j++)
    {
      if (nodes[i].adjacent[j])
      {
        std::cout << j + 1 << " ";
      }
    }
...
void Graph::mark_adjacent_nodes()
{
  ...
  for (int i = 0; i < RANK * RANK * RANK * RANK; i++)
  {
    ...
    int node_value = static_cast<int>(nodes[static_cast<std::size_t>(row * RANK * RANK + col)].value);

    if (node_value != DOT)
    {
      for (int j = 0; j < RANK * RANK; j++)
      {
        nodes[row * RANK * RANK + j].adjacent[node_value - 1] = true;
        nodes[j * RANK * RANK + col].adjacent[node_value - 1] = true;
      }
      ...
      for (int k = gridRow * RANK; k < (gridRow + 1) * RANK; k++)
      {
        ...
        {
          nodes[k * RANK * RANK + l].adjacent[node_value - 1] = true;
        }
      }
    }
  }
...
void Graph::solve()
{
  auto is_empty = [](Node &node)
  { return node.value == DOT; };
